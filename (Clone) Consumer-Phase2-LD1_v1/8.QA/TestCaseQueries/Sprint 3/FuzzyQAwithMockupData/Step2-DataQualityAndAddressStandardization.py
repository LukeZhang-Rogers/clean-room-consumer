...
keyword      = ['avenue', 'boulevard', 'boul', 'by-pass', 'centre', 'circle', 'circuit', 'concession', 'corners', 'court', 'crescent', 'cresent', 'crossing', 'cul-de-sac', 'diversion', 'drive', 'esplanade', 'estates', 'expressway', 'extension', 'freeway', 'gardens', 'glen', 'grounds', 'harbour', 'heights', 'highlands', 'highway', 'landing', 'limits', 'lookout', 'mountain', 'orchard', 'park', 'parkway', 'passage', 'pathway', 'place', 'plateau', 'point', 'private', 'promenade', 'range', 'ridge', 'road', 'route', 'square', 'street', 'subdivision', 'terrace', 'thicket', 'townline', 'turnabout', 'village', 'lane', 'close', 'loop', 'plaza', 'trail', 'st st', 'mount', 'av', 'chase', 'castle', 'gate', 'ct', 'grove', 'way', 'bl', 'cr']

lookupid     = ['ave', 'blvd', 'blvd', 'bypass', 'ctr', 'cir', 'circt', 'conc', 'crnrs', 'crt', 'cres', 'cres', 'cross', 'cds', 'divers', 'dr', 'espl', 'estate', 'expy', 'exten', 'fwy', 'gdns', 'glen&', 'grnds', 'harbr', 'hts', 'hghlds', 'hwy', 'landng', 'lmts', 'lkout', 'mtn', 'orch', 'pk', 'pky', 'pass', 'ptway', 'pl', 'plat', 'pt', 'pvt', 'prom', 'rg', 'ridge', 'rd', 'rte', 'sq', 'st', 'subdiv', 'terr', 'thick', 'tline', 'trnabt', 'villge', 'ln', 'cls', 'lp', 'pz', 'tr', 'st', 'mt', 'ave', 'ch', 'cs', 'ga', 'crt', 'gr', 'wy', 'blvd', 'cres']
...
shaw_consumer_contact             = mockup_path + '/shaw_contacts'
shaw_consumer_wireline_account    = mockup_path + '/shaw_wireline_accounts'
shaw_consumer_wireless_account    = mockup_path + '/shaw_wireless_accounts'
...
###################### Output Path #####################
cl_shaw_consumer_contact          = mockup_cl_path + '/shaw_contact'
cl_shaw_consumer_wireline_account = mockup_cl_path + '/shaw_wireline_account'
cl_shaw_consumer_wireless_account = mockup_cl_path + '/shaw_wireless_account'

cl_rogers_contact                 = mockup_cl_path + '/rogers_contact'
cl_rogers_wireline_account        = mockup_cl_path + '/rogers_wireline_account'
cl_rogers_wireless_account        = mockup_cl_path + '/rogers_wireless_account'
...
display(shaw_consumer_contact_df)

# COMMAND ----------

display(rogers_contact_df)

# COMMAND ----------

display(rogers_wireless_account_df.dtypes)
...
rogers_contact_df_temp = rogers_contact_df_temp.withColumn("city_cleansed", when((col("city").like("%\'%"))| (col("city").like('%-%')) | (col("city").like('% %')) | (col("city").like('%.%')) ,translate(col("city")," -'.",""))
                                                           .otherwise(col("city")))
rogers_contact_df_temp = rogers_contact_df_temp.withColumn("address_cleansed", when((col("city").like("%\'%")) | (col("city").like('%-%')) | (col("city").like('% %')) | (col("city").like('%.%')) ,expr("replace(address, city, city_cleansed)"))
...
rogers_contact_combined_add_df = rogers_contact_df_temp.join(rogers_contact_parsed_add_df, ['id']).drop('id')
...
rogers_contact_normalized_add_df = rogers_contact_normalized_add_df.withColumn('dq_first_name_cl', when((col('first_name_cl')== 'comp') | (col('first_name_cl')== 'unknown') | (col('first_name_cl')== 'blank'), lit(None)).otherwise(col('first_name_cl')))
rogers_contact_normalized_add_df = rogers_contact_normalized_add_df.withColumn('dq_last_name_cl', when((col('last_name_cl')== 'unknown'), lit(None)).otherwise(col('last_name_cl')))
...
rogers_wireline_account_combined_add_df = rogers_wireline_account_df_temp.join(rogers_wireline_account_parsed_add_df, ['id']).drop('id')
...
shaw_contact_df_temp = shaw_contact_df_temp.withColumn("city_cleansed", when((col("city").like("%\'%")) | (col("city").like('%-%'))  | (col("city").like('% %')) | (col("city").like('%.%')) 
                                                                             ,translate(col("city")," -'.",""))
                                                       .otherwise(col("city")))
shaw_contact_df_temp = shaw_contact_df_temp.withColumn("address_full", concat(col('ADDRESS'), lit(' '), col('city_cleansed'), lit(' '),  col('STATE'), lit(' '), col('ZIPCODE')))
...
shaw_contact_combined_add_df = shaw_contact_df_temp.join(shaw_contact_parsed_add_df, ['id']).drop('id')
...
shaw_wireline_account_df_temp = shaw_wireline_account_df_temp.withColumn("service_city_cleansed", when((col("service_city").like("%\'%")) | (col("service_city").like('%-%')) | (col("service_city").like('% %')) | (col("service_city").like('%.%'))
                                                                                                       ,translate(col("service_city")," -'.",""))
                                                                         .otherwise(col("service_city")))
shaw_wireline_account_df_temp = shaw_wireline_account_df_temp.withColumn("service_address_temp", concat(col('service_address'), lit(' '), col('service_city_cleansed'), lit(' '), col('service_province'), lit(' '), col('service_postal_code')))
...
shaw_wireline_account_combined_add_df = shaw_wireline_account_df_temp.join(shaw_wireline_account_parsed_add_df, ['id']).drop('id')
...
display(rogers_contact_normalized_add_df)

# COMMAND ----------

display(shaw_contact_normalized_add_df)
